...
for (Variabele in Componenten) {
  ...
  ggsave(figuur,
         filename = paste0("./exports/boxplots/", "Kalmthout_",Variabele, "_bw",".jpg"))
}
```
...
# plots genereren voor 1 variabele
...
maak_plot_1var(data, "TP", "Totale fosfor Kalmthout", yas = "Meetwaarde (mg P/L)")

# plot TN
maak_plot_1var(data, "TN", "Totale stikstof Kalmthout", yas = "Meetwaarde (mg N/L)")
...
for (ID in locaties) {
  # subset voor locatie
  N_anorg_ID <- data_met_N_anorg %>%
  filter(AquaComponent %in% c("NH4_N", "NO3_N", "NO2_N", "N_anorg") & CODE == ID)
  ...
  print(figuur)
  ggsave(plot = figuur,
         filename = paste0("./exports/N_anorg(2)/", ID,"_N_anorg2", ".jpg"), width = 1080, height = 720, units = "px", scale = 3)
}
...
maak_plot_1var(data, "NH4_N", "Ammonium-stikstof Kalmthout", yas = "Meetwaarde (mg N/L)")
maak_plot_1var(data, "NO3_N", "Nitraat-stikstof Kalmthout", yas = "Meetwaarde (mg N/L)")
maak_plot_1var(data, "NO2_N", "Nitriet-stikstof Kalmthout", yas = "Meetwaarde (mg N/L)")
...
maak_plot_1var(data, "pH_veld", "Zuurtegraad Kalmthout (MKN minimum - maximum)", yas = "Meetwaarde (pH)") + geom_hline(yintercept = 4.5, color = "black", linetype = "dashed", linewidth = 1) +
    geom_hline(yintercept = 6.5, color = "black", linetype = "dashed", linewidth = 1)
```
...
maak_plot_1var(data, "absorptie_440", "Absorptie (440 nm)", yas = "Meetwaarde (ABS)")
...
maak_plot_1var(data, "chl_a", "chlorofyl-a Kalmthout", yas = "Meetwaarde (µg/L)")
...
# zwevende stof
for (ID in locaties) {
  figuur <- maak_plot_1var(data %>% filter(CODE %in% ID), "ZS_105" , paste("Zwevende stof (105°C)", ID), yas = "Meetwaarde (g/L)") +
  scale_y_continuous(breaks = pretty_breaks(n = 10, min.n = 10))
  print(figuur)
  ggsave(plot = figuur,
...
maak_plot_1var(data, "ZS_105", "Zwevende stof (105°C) Kalmthout", yas = "Meetwaarde (g/L)")

## zwevende stof ZS550 - ZS105
for (ID in locaties) {
  figuur <- maak_plot(data %>% filter(CODE %in% ID), c("ZS_105", "ZS_550") , paste("Zwevende stof (550 & 105°C)", ID), yas = "Meetwaarde (g/L)") +
  scale_y_continuous(breaks = pretty_breaks(n = 10, min.n = 10))
  print(figuur)
  ggsave(plot = figuur,
...
# secchi-diepte
for (ID in locaties) {
  figuur <- maak_plot_1var(data %>% filter(CODE %in% ID), "SD_veld" , paste("Secchi-diepte", ID, "(+ MKN  zomerhalfjaargemiddelde)"), yas = "Meetwaarde (m)") +
  scale_y_continuous(breaks = pretty_breaks(n = 10, min.n = 10)) +
...
maak_plot_1var(data, "SD_veld", "Secchi-diepte Kalmthout (+ MKN Zomerhalfjaargemiddelde)", yas = "Meetwaarde (m)") + 
  geom_hline(yintercept = 0.9, color = "black", linetype = "dashed", linewidth = 1)
