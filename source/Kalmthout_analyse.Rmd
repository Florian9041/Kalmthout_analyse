...
maak_plot(data, "T.P", " Totale fosfor Kalmthout", yas = "Meetwaarde (mg P/L)")

# TP voor alle locaties
data %>%
  filter(Component %in% "T.P") %>%
  ggplot(aes(x = FieldSamplingDate, y = ResultFormattedNumeric, colour = CODE)) +
  geom_line(size = 1) +
