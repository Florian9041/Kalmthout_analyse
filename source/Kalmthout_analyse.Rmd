...
for (ID in locaties) {
  figuur <- maak_plot_1var(data %>% filter(CODE %in% ID), "T.P" , paste("Totale fosfor T.P", ID), yas = "Meetwaarde (mg P/L)") + geom_hline(yintercept = 0.03, color = "black", linetype = "dashed", size = 1) +
...
```{r}
for (ID in locaties) {
  figuur <- maak_plot_1var(data %>% filter(CODE %in% ID), "T.N" , paste("Totale stikstof T.N", ID), yas = "Meetwaarde (mg N/L)") + geom_hline(yintercept = 0.03, color = "black", linetype = "dashed", size = 1) +
  scale_y_continuous(breaks = pretty_breaks(n = 10))
  print(figuur)
  ggsave(plot = figuur,
         filename = paste0("./exports/TP/", ID,"_TP", ".jpg"), width = 1080, height = 720, units = "px", scale = 3)
